...
class ConEntrada:
    ...
    def datos_tabla_fechas(self, fecha_inicial, fecha_fin):
        # lista = [("1", "11/05/21", "00/00/0000", "compra", "50", "holaaa esto es una notaa", "link", "link"),
        #         ("2",  "11/05/21", "00/00/0000", "compra", "50", "holaaa esto es una notaa", "link", "link"),
        #         ("3",  "11/05/21", "00/00/0000", "compra", "50", "holaaa esto es una notaa", "link", "link")]
        obj_conexion = conn.dbconexion()
        ...
        cursor = conexion.cursor()
        entidades=fecha_inicial.cget("text"),fecha_fin.cget("text")
        cursor.execute("SELECT op_id_operacion,op_fecha_entrada,op_fecha_salida,op_operacion,op_gan_per,op_nota,op_ruta_imagen_ent,op_ruta_imagen_sal from tb_operaciones WHERE op_fecha_entrada BETWEEN ? AND ?",entidades)
        lista = cursor.fetchall()
